"""Agent stream server (:9100) — POC 5.

Runs `claude -p --output-format stream-json` (or a credential-free demo producer)
...
given run via Last-Event-ID / ?from=<id>.

POC-5 change: the Claude credential is NOT sent in the request. The VM fetches it
from AWS Secrets Manager itself, using the runtime role attached via the
RunMicrovm executionRoleArn (boto3's default credential chain picks up the
ambient role credentials).

Endpoints (over the MicroVM's authenticated ingress on :9100):
  POST /run    {"task": "..."}      -> start a real Claude run (VM fetches the credential)
               {"demo": true}       -> start a canned producer (no credential)
...
def fetch_credential() -> str:
    """Fetch the Anthropic API key from Secrets Manager using the VM's runtime role.
    SecretString is a plain string (not JSON), used verbatim."""
    sm = boto3.client("secretsmanager", region_name=AWS_REGION)
    return sm.get_secret_value(SecretId=SECRET_NAME)["SecretString"].strip()


def credential_env(cred: str) -> dict:
    """Pick the right env var by credential type: OAuth tokens (sk-ant-oat...) use
    CLAUDE_CODE_OAUTH_TOKEN, API keys (sk-ant-api...) use ANTHROPIC_API_KEY. The
    secret is named 'api-key' but may actually hold either, so detect, don't assume."""
...
class Handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        if body.get("demo"):
            ...
            threading.Thread(target=run_claude, args=(stream, body["task"]), daemon=True).start()
        else:
            self.send_error(400, "task (or demo) required")
            return
